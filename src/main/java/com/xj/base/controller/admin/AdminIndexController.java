...
public class AdminIndexController extends BaseController{
	...
	public String index(Model model){
		...
		for (Resource resource : roleResources) {
			...
			if ("报销编辑".equals(resource.getName())) {
				...
				builder.add("status", Operator.eq.name(), "0");
				 Sharecount = (int)ShareService.count(builder.generateSpecification());
				count += Sharecount;
				if ( 0!= Sharecount) {
					Sharetip = "尊敬的 " + user.getNickName() + "您有" + Sharecount + "条报销需要审核";
				}
				status = "1";
				break;
			}
		...
		model.addAttribute("Sharecount", String.valueOf(Sharecount));
		model.addAttribute("count", count);
		model.addAttribute("status", status);
		model.addAttribute("schetip", schetip);
		model.addAttribute("Sharetip", Sharetip);
